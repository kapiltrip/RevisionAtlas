...
For each page, first state the physical charge or conducting path, then the valid operating region and assumptions, and only then the equation. Check signs and limiting cases. End by explaining one circuit consequence: gain, logic level, delay, power, area, or noise. Use the global [revision method](../guides/REVISION_METHOD.md) for session structure and spaced reviews.
