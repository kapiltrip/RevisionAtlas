`timescale 1ns/1ps

module divide_by_5 (
    input  wire clk,
    input  wire reset,
    output wire clk_out_10,
    output wire clk_out_20,
    output wire clk_out_30,
    output wire clk_out_40,
    output wire clk_out_50,
    output wire clk_out_60,
    output wire clk_out_70,
    output wire clk_out_80,
    output wire clk_out_90
);

    reg [2:0] posphase;
    reg [2:0] negphase;

    wire one_cycle_posphase;
    wire one_cycle_negphase;
    wire two_cycles_posphase;
    wire two_cycles_negphase;
    wire three_cycles_posphase;
    wire three_cycles_negphase;
    wire four_cycles_posphase;
    wire four_cycles_negphase;

    assign one_cycle_posphase    = (posphase < 3'd1);
    assign one_cycle_negphase    = (negphase < 3'd1);
    assign two_cycles_posphase   = (posphase < 3'd2);
    assign two_cycles_negphase   = (negphase < 3'd2);
    assign three_cycles_posphase = (posphase < 3'd3);
    assign three_cycles_negphase = (negphase < 3'd3);
    assign four_cycles_posphase  = (posphase < 3'd4);
    assign four_cycles_negphase  = (negphase < 3'd4);

    always @(posedge clk or posedge reset) begin
        if (reset)
            posphase <= 3'd4;
        else if (posphase == 3'd4)
            posphase <= 3'd0;
        else
            posphase <= posphase + 1'b1;
    end

    always @(negedge clk or posedge reset) begin
        if (reset)
            negphase <= 3'd4;
        else
            negphase <= posphase;
    end

    // One divide-by-5 period contains ten input half-cycles.
    // All valid duty-cycle waveforms are generated simultaneously.
    assign clk_out_10 = reset ? 1'b0 :
                        one_cycle_posphase & ~one_cycle_negphase;
    assign clk_out_20 = reset ? 1'b0 :
                        one_cycle_posphase;
    assign clk_out_30 = reset ? 1'b0 :
                        one_cycle_posphase | one_cycle_negphase;
    assign clk_out_40 = reset ? 1'b0 :
                        two_cycles_posphase;
    assign clk_out_50 = reset ? 1'b0 :
                        two_cycles_posphase | two_cycles_negphase;
    assign clk_out_60 = reset ? 1'b0 :
                        three_cycles_posphase;
    assign clk_out_70 = reset ? 1'b0 :
                        three_cycles_posphase | three_cycles_negphase;
    assign clk_out_80 = reset ? 1'b0 :
                        four_cycles_posphase;
    assign clk_out_90 = reset ? 1'b0 :
                        four_cycles_posphase | four_cycles_negphase;

endmodule
