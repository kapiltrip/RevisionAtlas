...
The upper sketch shows why addressing matters on a shared bus. The controller broadcasts a target address after START; every target can observe the bits, but only the matching target should acknowledge and continue. In the normal 7-bit format, the first byte contains seven address bits followed by one direction bit. Ten-bit addressing uses a two-byte address sequence; it is not simply a ten-bit field placed where the seven-bit field used to be.
