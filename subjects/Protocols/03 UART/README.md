...
The opening classification separates on-chip interconnects such as AMBA, APB, and AXI from peripheral serial interfaces such as SPI, UART, and I2C. A processor may access a UART's control and data registers through APB or AXI, while the UART independently converts the parallel register contents into a timed serial waveform at its external TX pin.
...
The lower annotations identify common rates such as 9600 and 115200. Baud is measured in symbols per second, whereas bit rate is measured in bits per second. For ordinary binary NRZ UART, each symbol interval carries one bit, so the numerical values coincide. This equality follows from the signaling method and is not part of the general definitions.
...
The first equation converts baud rate into the corresponding bit interval. For one-bit-per-symbol UART,
...
The next equation relates the external bit interval to a 50 MHz system clock. One system-clock period is
...
The non-integer result imposes an implementation constraint: an integer divider cannot generate exactly 9600 baud from exactly 50 MHz. A fixed interval of 5208 cycles produces approximately 9600.614 baud, while 5209 cycles produces approximately 9598.77 baud. The corresponding errors are about $+0.0064\%$ and $-0.0128\%$. A fractional accumulator can alternate integer interval lengths to reduce the long-term average error further.

The lower waveform distinguishes the system-clock period from the UART bit period. The 20 ns system clock remains continuous; a counter converts thousands of system-clock cycles into a one-cycle `baud_tick`. The transmitter advances to the next frame bit only when this enable is asserted. Keeping a single system clock with clock-enable pulses also avoids the clock-tree and timing-analysis complications of a fabric-derived clock.
...
The upper diagram separates the parallel system-side interface from the serial line. The transmitter accepts a parallel byte and emits one framed bit at a time; the receiver samples the serial input and reconstructs a parallel byte. Because the system clock is much faster than the line rate, counters or phase accumulators generate the required timing enables.
...
The lower diagram assigns parallel-to-serial conversion to TX and serial-to-parallel conversion to RX. The baud generator supplies timing enables, while the state machines determine the active frame field.
...
The BRG block converts the fast system clock into timing enables for the transmitter and receiver. A synchronous RTL implementation normally emits one-system-clock-wide pulses such as `tx_en` and `rx_en`; downstream logic remains clocked by the main `clk` and advances only while the corresponding enable is asserted. This avoids introducing uncontrolled derived clocks into FPGA routing.
...
Once the START midpoint is confirmed, the receiver waits 16 additional sample ticks to reach the center of data bit 0, stores that value, and repeats at 16-tick intervals for bits 1 through 7. The waveform arrows therefore denote an initial half-bit alignment delay followed by full-bit spacing between data samples. A robust implementation may majority-vote several central samples instead of relying on a single sample.
...
The handwritten condition `sample == 15 && index == 7` identifies the eighth data-bit sampling event if the phase counter's terminal value corresponds to one full bit. The FSM must then transition to `STOP` rather than declare a complete byte immediately, because STOP must be sampled HIGH one bit later. Since `temp[index] <= rx` updates after the edge, an accompanying `data_out <= temp` receives the old `temp`; the final bit must be inserted explicitly or publication must be delayed until STOP validation.
